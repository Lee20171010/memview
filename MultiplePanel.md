...
We will pre-register 10 views. This is a static requirement of VS Code extensions.
...
*   **Views**:
    ...
    *   `memory-view.memoryView6` (MEMORY6) - Controlled by `memory-view:showPanel6`
    *   `memory-view.memoryView7` (MEMORY7) - Controlled by `memory-view:showPanel7`
    *   `memory-view.memoryView8` (MEMORY8) - Controlled by `memory-view:showPanel8`
    *   `memory-view.memoryView9` (MEMORY9) - Controlled by `memory-view:showPanel9`
...
*   **Factory/Manager**: A `MemViewPanelManager` class will be responsible for creating and registering providers for each of the 10 view IDs.
...
*   **Mechanism**:
    ...
    *   **Error Handling**: If all 10 panels are already visible:
...
*   **Visibility**:
    ...
    *   **Secondary Panels (MEMORY1-9)**: Visible.
*   **Mechanism**:
    ...
    *   Extension Logic:
        ...
        3.  **Resource Cleanup**:
            *   Calls `manager.clearAllDocuments()` to remove all documents from memory.
            *   Calls `updateWebviewDocs()` to clear the frontend UI and **save the empty state** to `workspaceState`.
            *   This ensures that when the panel is re-opened, it starts fresh (empty).
        4.  **Re-opening**: When the panel is re-opened, it starts as a fresh panel.
...
*   **Resource Cleanup (Memory Leaks)**:
    ...
    *   **Handling**:
        *   We explicitly `clearAllDocuments()` associated with that panel.
        *   This ensures all event listeners (e.g., `DebuggerTracker` subscriptions) attached to those documents are unsubscribed, preventing memory leaks and CPU usage from background updates.
        *   The state is saved to disk before clearing, allowing restoration.
...
2.  **Update `package.json`**: **Completed**. 9 new views added (Total 10).
3.  **Update `MemViewPanelProvider`**: **Completed**. Supports multiple instances and registers all 10 views.
4.  **Update UI (React)**: **Completed**. "+" and "X" buttons added to toolbar.
5.  **Implement Logic**: **Completed**. "+" finds next available view, "X" hides the panel, saves state, and clears resources.
...
3.  **Step 3**: Implement `MemViewPanelManager` (via static `Providers` list) to manage the 10 provider instances. **(Done)**
...
*   **`MemViewExtension` (Backend)**:
    ...
    *   `setContexts` initializes the visibility contexts for the new panels (`memory-view:showPanel1`...`9`) to `false` by default.

*   **`MemViewPanelProvider.register`**:
    *   **Status**: **Updated**. Registers all 10 views and populates the `Providers` list.
